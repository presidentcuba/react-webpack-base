import { CACHE_OBJECT_STORENAME } from '@/utility/constants'
	...
	if (dateString) {
		try {
			...
			let dateStringFormated =
				ConvertStr(d.getMonth() + 1) + '/' + ConvertStr(d.getDate()) + '/' + d.getFullYear().toString()
			if (!notGetTime) dateStringFormated += ' ' + ConvertStr(d.getHours()) + ':' + ConvertStr(d.getMinutes())
...
export function formatDate(dateString, notGetTime) {
	if (dateString) {
		try {
			const d = new Date(Date.parse(dateString))
			let dateStringFormated =
				ConvertStr(d.getDate()) + '/' + ConvertStr(d.getMonth() + 1) + '/' + d.getFullYear().toString()
			if (!notGetTime) dateStringFormated += ' ' + ConvertStr(d.getHours()) + ':' + ConvertStr(d.getMinutes())
			return dateStringFormated
		} catch (error) {
			return dateString
...
export function formatDateCusNew(dateString, notGetTime) {
	if (dateString) {
		try {
			const d = new Date(Date.parse(dateString))
			let dateStringFormated =
				d.getFullYear().toString() + '-' + ConvertStr(d.getMonth() + 1) + '-' + ConvertStr(d.getDate())
			if (!notGetTime) dateStringFormated += ' ' + ConvertStr(d.getHours()) + ':' + ConvertStr(d.getMinutes())
			return dateStringFormated
		} catch (error) {
			return dateString
