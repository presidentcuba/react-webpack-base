...
System.Type.RegisterClass = function (typeName, baseType, interfaceTypes) {
	...
	o.prototype.GetType = function () {
		return new System.Type(typeName)
	}
}
	...
	this.Add = function (node, eventName, eventHandler, capture) {
		...
		if (node) {
			if (typeof capture !== 'boolean') {
				...
				if (eventHandler.GetType && eventHandler.GetType().FullName === 'System.EventHandler')
					eventHandler = eventHandler.InvokeNative
...
System.DateTime.Expressions = {
	...
	Utc: new RegExp(
		'([0-9][0-9][0-9][0-9])-(0[1-9]|1[012])-(0[1-9]|[12][0-9]|3[01])' +
			'[T ]' +
			'([01][0-9]|[2][0123]):([012345][0-9]):([012345][0-9])'
	),
}
...
System.DateTime.ExpressionUtc = new RegExp(
	System.DateTime.ExpressionUtcDate.toString() + '[T ]' + System.DateTime.ExpressionUtcTime.toString()
)
...
System.DateTime.IsDate = function (valDate) {
	...
	results = ''
	// Get Day, Month, Year;
	if (!System.DateTime.Expression.test(dateString))
		return 'Invalid! <span style="color: gray;">Format: mm/dd/yyyy</span>'
...
System.Web.UI.Console = function (id, context) {
	...
	this.SetStyle = function (name) {
		...
		this.returnIdent = function () {
			return me.CurrentIdent
		}
		this.returnDate = function () {
			return '<span style="color: #A0A0A0;">' + new Date().ToString(me.TimeStamp) + ': </span>'
		}
		this.Font = ''
...
System.Web.Events.Add = function (sourceObjectId, eventName, objectName, delegateName) {
	...
	var delegateScript =
		eventId +
		' = function(e){ var e = e ? e : window.event; var sender = e.target ? e.target : e.srcElement; ' +
		objectName +
		'.OnEvent(sender,e); }'
	var eventScript = ''
	if (this.attachEvent) {
		eventScript =
			'document.getElementById("' + sourceObjectId + '").attachEvent("on' + eventName + '", ' + eventId + ');'
	} else {
		eventScript =
			'document.getElementById("' + sourceObjectId + '").addEventListener("' + eventName + '", ' + eventId + ',true);'
	}
	Trace.Write(
		this.GetType().Name + '(' + sourceObjectId + ', ' + eventName + ', ' + objectName + ', ' + delegateName + ');',
		1
	)
...
System.Web.Events.Remove = function (sourceObjectId, eventName, objectName, delegateName) {
	...
	if (this.detachEvent) {
		eventScript =
			'document.getElementById("' + sourceObjectId + '").detachEvent("on' + eventName + '", ' + eventId + ');'
	} else {
		eventScript =
		...
		if (me.HttpRequest.readyState === 4) {
			...
			Trace.Write(
				id +
					...
					scriptRunTime.getMinutes() +
					':' +
					scriptRunTime.getSeconds() +
					'.' +
					scriptRunTime.getMilliseconds()
			)
		...
		if (me.TimerId === -1) {
			...
			Trace.Write(
				this.id +
					'.Resume(' +
					me.AutoReset +
					') // Time passed from: LastStop = ' +
					diff +
					'; Difference with iterval = ' +
					waitTime
			)
		...
		if (bestNo === -1) {
			...
			Trace.Write(
				'// Local Time: ' +
					...
					' - Server Time: ' +
					serverTime.ToString('yyyy-MM-dd HH:mm:ss.fff')
			)
			...
			e['Message'] =
				'Time Difference between your PC and ' +
				this.TestType +
				' is: ' +
				e.Difference +
				' sec. [Bias: ±' +
...
System.Diagnostics.TraceListener = function (obj) {
	if (arguments[0] === System.Diagnostics.TraceInternal || arguments[0].GetType().Name === 'TraceListener')
		return arguments[0]
}
